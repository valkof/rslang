...
export class DifficultySelector extends Component {
  constructor(parent: HTMLElement, style: string, title: string, text: string) {
    super(parent, 'div', [`${style}-dificulty`]);
    const dificultyTitle = new Component(this.root, 'h2', [`${style}-dificulty__title`]);
    dificultyTitle.root.textContent = title;
    const dificultyDescription = new Component(this.root, 'h3', [`${style}-dificulty__description`]);
    dificultyDescription.root.textContent = text;
    const levelContainer = new Component(this.root, 'div', [`${style}-dificulty__level-container`]);
    for (let i = 0; i < 6; i++) {
      const item = new Component(levelContainer.root, 'div', [
        `${style}-dificulty__level-container__item`,
