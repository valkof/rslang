...
export default class StatisticPopup extends Component {
  ...
  private renderAnswers(answers: TWord[], iCorrect: boolean) {
    ...
    new Component(this.root, 'h2', ['statistic-title'], `${text} ${answers.length}`);
  ...
  private renderRow(word: TWord, index: number, correct: boolean) {
    ...
    new Component(container.root, 'h3', ['row-index'], `${index}`);
    new Component(container.root, 'h3', ['row-word'], ` ${word.word}`);
    new Component(container.root, 'h3', ['row-word'], ` ${word.transcription}`);
    new Component(container.root, 'h3', ['row-word'], ` ${word.wordTranslate}`);
  }
